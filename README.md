...
1. 円滑なコミュニケーション．
2. 読みやすい(追いやすい)コーディング．
3. 適切なPHPDoc・タイプヒンティングの記述．

## :heart_eyes: 好きなこと・もの
1. お金．
2. 日々の生活を改善すること(タスクの自動化など)．
3. ショートカットキーを使い倒すこと．
...
1. 円滑なコミュニケーションの元，社内運用者からの要望を要件に落とし込み実装すること．
2. 新卒エンジニアのメンター業務や採用活動へのサポート業務．

<details>
   <summary><b>詳細はこちら :point_right:</b></summary>
   <div>
        <p>新卒1期で入社した私がエンジニア3年目のタイミングで，新卒エンジニアのメンターを担当した．</p>
        <p>Slackでも口頭でもいつでも相談してよいとしているものの，自発的な相談が苦手な人を担当したため，定期的にポーリングする形で吸い出したり，相談を受けるときはできる限りPCを閉じて目を見て話を聞くなど，丁寧なコミュニケーションを心がけていた．</p>
    </div>
</details>

---

## :office: 2018/12 - 2021/07 : 株式会社MATCHA
- 職務：Webアプリケーションエンジニア / サーバサイドエンジニア / フロントエンドエンジニア
- 雇用形態：正社員

---

### :computer: 2018/12 - 2021/07 : インバウンドメディア「[MATCHA](https://matcha-jp.com)」の機能追加・保守運用・改修
...
2. 負荷軽減を目的としたクエリチューニング
3. 施策リリース後の効果測定を基にした後続施策の実施可否等の検証等
...
<details>
   <summary><b>詳細はこちら :point_right:</b></summary>
   <div>
        <p>現行システムの都合や過去の仕様に基づき，実装が困難ケースや実装コストに対する施策効果が伴わないケースにおいて，翻訳依頼やデザイン，マークアップ，外部委託者の作業完了後にシステムに側に相談されることが定期的に発生する．</p>
        <p>その際に，宙に浮きっぱなしのMTGがたくさん行われる中で，もうこれは収拾が付かないと判断した最に，システム側で実現可能な代替案を模索し提示し，確実に着地させることを行ってきた．</p>
    </div>
</details>
...
### :computer: 2020/04 - 2021/04 : インバウンドメディア「[MATCHA](https://matcha-jp.com)」のリビルド開発
...
### :computer: 2022/05 - 2022/08 : 請求明細出力機能の開発
- ラクスルエンタープライズでは，ラクスルの請求先で支払いでき，請求先ごとに請求書が発行される．
- 上記のそれぞれの請求書を1つに集約したExcelを出力する機能の開発をおこなった．
...
#### :raising_hand: 主な担当箇所
1. 要件定義，設計，実装，QA．
2. 各サービス基盤との相談，連携，QAなどの全体調整．

#### :muscle: 特に尽力したこと
1. 手動で作成していたExcelのオペレーションの把握．
2. 各サービスの連携方法とドメイン知識のキャッチアップ．
3. 各サービスのスキーマ統一に伴う調整と連携．

<details>
   <summary><b>詳細はこちら :point_right:</b></summary>
   <div>
        <p>ラクスルでは複数のECサイトがあり，ほとんどの商品をラクスルエンタープライズで購入可能である．ECサイトが分かれてるため，情報を集約して出力すると，差異が顕在化してしまうため，全てのECサイトにおいて文言，料金，オプションなど，細部の表記の統一を実施した．</p>
        <p>上記を行うため，各ECのドメイン知識のキャッチアップ尽力した．</p>
    </div>
</details>
...
#### :muscle: 特に尽力したこと
- OpenID Connectの仕様の理解と実装．
- 通常ユーザーと異なり，Resource Serverの情報を基にした認可処理の実装．

<details>
   <summary><b>詳細はこちら :point_right:</b></summary>
   <div>
        <p>専用注文サイト機能に，外部のIdPとResource Serverを基に入退会処理と認証認可の機能を開発した．</p>
        <p>通常ユーザーと異なり企業が定めた登録条件が存在している．その条件を満たしているかをResource Serverを確認し都度認可をしている．</p>
        <p>また，IdPのリフレッシュトークンを使用してアクセストークンを再発行し，注文途中などでログインセッションが無効にならないよう，ユーザーのエクスペリエンスの向上を考慮した．</p>
    </div>
</details>
...
- ラクスルのシステム担当者やサポート担当者が，任意のユーザーでログインして，企業の設定・編集・削除を行える機能の開発．
- ユーザー，オペレータの操作ログを残し，代理ログインによる操作かどうかを判別できるようにした．
- この機能の実現により，サポート担当者がユーザーの操作ログを確認し，問題解決に向けたアクションを取りやすくなった．
...
#### :raising_hand: 主な担当箇所
1. 全体要件定義，設計，実装，QA．
2. オペレーターへの認証認可の部分の実装．
3. ユーザー，オペレータの操作ログの残し方の設計．

#### :muscle: 特に尽力したこと
- ユーザー，オペレーターの操作ログの残し方の設計．
- 管理画面から有効なオペレーターのセッションから任意のユーザーのセキュアなセッションの発行方法の検討と実装．

<details>
   <summary><b>詳細はこちら :point_right:</b></summary>
   <div>
        <p>顧客問い合わせや調査対応のため，オペレーターが代理でログインして設定変更した場合，ページの動きは対象ユーザーとして動くが，値を変更した人はオペレーターとして記録したい．</p>
        <p>そのため，モデル単位で変更のログを残すことができる，auditedというgemを採用して実現した．</p>
        <p>また，管理画面からユーザーマイページへは，JWTを用いてセキュアにセッションの発行を実現した．</p>
    </div>
</details>
